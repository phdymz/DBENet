...
from util.misc import extract_features, extract_features_kp
...
import torch
from model.kpfcgf import ResUNetBN4C
...
def extract_features_batch_kp(
  ...
  for scene in folders:
    ...
    for i in tqdm.tqdm(range(len(files))):
      ...
      xyz_down, feature = extract_features_kp(
          model,
...
if __name__ == '__main__':
  ...
  target_path = '/media/ymz/2b933929-0294-4162-9385-4fe3eec72189/distillation/des/att'
  checkpoint_path = '/home/ymz/桌面/Distillation/outputs/best.pth'
  ...
  if args.extract_features:
    ...
    #acquire neighborhood limits
      config.neighborhood_limits = np.load('neighborhood_3dmatch.npy')

      with torch.no_grad():
          # 产生每个场景抽取特征的描述符，以及对应点
          extract_features_batch_kp(
              model=model,
              config=config,
